...
public class ItemsAspectTest {
    ...
    public void testDiscount() throws JsonProcessingException {
        ...
        lineItem.withPriceType("some_price_type")
                .withItemPrice(20d)
                .withCurrency("RSD")
                .withItemCount(1d)
                .withDiscountAmount(250d)
                .withDiscountPercentage(22.54);

        items.mergeItemLine(lineItem);
        show.withAspect(items);

        Assert.assertEquals(show.toJSON(), "{\"entity_ref\":\"Show/1\",\"aspects\":{\"items\":[{\"price_type\":\"some_price_type\",\"item_price\":20.0,\"currency\":\"RSD\",\"item_count\":1.0,\"discount_amount\":250.0,\"discount_percentage\":22.54}]}}");
    }
}
